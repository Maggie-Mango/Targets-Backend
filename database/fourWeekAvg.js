...
SELECT
    ...
    CASE 
        ...
        WHEN CASES IN ('UNLINKED_REFUNDS') THEN 10
        WHEN CASES IN ('LBU') THEN 11
        WHEN CASES IN ('BF') THEN 12
        WHEN CASES IN ('EF') THEN 13
    END AS caseKey,
    CASE 
...
FROM (
    SELECT
        CASE
            ...
            WHEN SUBROUTE IN ('UNLINKED_REFUND_CASE_REVIEW_LOW', 'UNLINKED_REFUND_CASE_REVIEW_HIGH') THEN 'UNLINKED_REFUNDS'
    ...
    UNION ALL
    SELECT 
        CASE WHEN CASE_TYPE = 'expiring_freeze' THEN 'EF' ELSE CASE_TYPE END AS CASES,
        TO_CHAR(created_at, 'DY') AS DAY,
        DATE(created_at) AS DATE,
    ...
    AND "group" IN ('tr')
    AND COUNTRY_CODE IN ('US')
    AND CASE_TYPE = 'expiring_freeze'
    AND USER_TOKEN IS NOT NULL
    GROUP BY 1, 2, 3
    ORDER BY DATE ASC
)
GROUP BY 1, 2, 4
ORDER BY caseKey, dayKey asc
`
